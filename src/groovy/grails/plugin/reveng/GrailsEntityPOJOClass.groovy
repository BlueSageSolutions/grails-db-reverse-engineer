...
//		}
	...
	String renderConstraints() {
		...
		getAllPropertiesIterator().each { Property property ->
			...
			if (property.columnSpan == 1) {
				...
				if (column.length && column.length != Column.DEFAULT_LENGTH && !property.name.toLowerCase().contains('date')) {
	...
	String getJavaTypeName(Property p, boolean useGenerics) {
        ...
        // installedOn is from the schema_version table, generated by Flyway
        if (p.name.toLowerCase().contains('datetime') || p.name.equalsIgnoreCase('installedOn')) {
            'Instant'
